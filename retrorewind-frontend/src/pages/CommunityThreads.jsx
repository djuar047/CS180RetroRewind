...
export default function CommunityThreads({ auth }) {
  const userId = auth?.userId;
  console.log("auth in CommunityThreads:", auth, "userId:", userId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const resList = await fetch("http://127.0.0.1:5000/threads");
      const threadsData = await resList.json();
      setThreads(Array.isArray(threadsData) ? threadsData : []);
    } catch (err) {
      console.error("Error creating thread:", err);
      alert("Server error creating thread.");
    }
  };
...
const saveEdit = async () => {
  ...
  await fetch(`http://127.0.0.1:5000/threads/${editingId}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ content: editContent, user_id: userId }),
  });
...
const deleteThread = async (thread, e) => {
  ...
  await fetch(`http://127.0.0.1:5000/threads/${thread.id}`, {
    method: "DELETE",
    headers: { "Content-Type": "application/json" },
...
<div className="space-y-4">
  {threads.length === 0 ? (
    ...
    threads.map((t) => {
      const isOwner = userId && t.user_id === userId; // 👈 check owner
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              ...
              <p className="text-sm text-zinc-500 mt-1">
                Posted by {t.username || `User ${t.user_id || "Unknown"}`}
              </p>
